...
import com.example.demo.repositoryDAO.PostRepository;
import com.example.demo.repositoryDAO.ProduitRepository;
import com.example.demo.repositoryDAO.RoleRepository;
...
public class Controlleur {
	    ...
	    public String updateAccountStatus(@RequestParam("action") String action,
	                                      @RequestParam("userId") Long userId) {
	        User user = userService.getUserById(userId);
	        System.out.println("didiiiiiiiiiiiiiii"+ user.toString());

	        if (user != null) {
	        	
	        	System.out.println("dOOOOOOOOOOOOOOOOOO"+ user.toString());
	            // Check the action value and update the status accordingly
	            if ("activate".equals(action)) {
	                user.setStatusUser(1); // Set the status for activation
	            } else if ("deactivate".equals(action)) {
	                user.setStatusUser(0); // Set the status for deactivation
	            }
			
			/*User user = userService.getUserById(userId);
			if ("activate".equals(action)) {
		        userService.updateAccountStatus(userId, 1);
		    } else if ("deactivate".equals(action)) {
		        userService.updateAccountStatus(userId, 0);
		    }*/
	   ...
	   public String datasetAdmin(Model model, HttpSession session) {
		   User loggedInUser = (User) session.getAttribute("loggedInUser");
		    if (loggedInUser != null) {
		        ...
		        List<Domaine> domains = domainService.getAllDomains();
	    ...
	    public String ajouterDataset(HttpSession session,
	     ...
	     // Vérifier si l'utilisateur est connecté
	        if (loggedInUser != null) {
	            ...
	            Domaine domain = domainService.getDomainByIdDomain(domainId); // Assurez-vous d'avoir cette méthode dans votre service
	   ...
	   public ResponseEntity<String> updateDataset(@RequestBody Map<String, String> updatedDataset, Model model) {
	       ...
	       if (optionalDataset.isPresent()) {
	           ...
	           if (updatedDataset.get("id_domain") != null) {
	               Domaine updatedDomain = domainService.getDomainByIdDomain(Long.parseLong(updatedDataset.get("id_domain")));
	           ...
	           if (updatedDataset.get("id_workspace") != null) {
	               Workspace updatedWorkspace = workspaceService.getWorkspaceById(Long.parseLong(updatedDataset.get("id_workspace")));
	   ...
	   @Autowired
	    private DomainRepository domaineRepository;
	   
	   @Autowired
	    private PostRepository posteRepository;
	   ...
	   public String processRegistration(
			   ...
			   if (motDePasse == null || !motDePasse.equals(confirmMotDePasse)) {
			    ...
			    try {
			        ...
			        Domaine domaines = domaineRepository.findByIdDomain(Long.parseLong(domaine));
			        Poste poste = posteRepository.findByIdPoste(Long.parseLong(posteStr));
			        Role role = roleRepository.findByIdRole(Long.parseLong(roleStr));
			        
			        user.setPoste(poste);
			        user.setRole(role);
			        user.setDomaine(domaines);
	   ...
	   public String ajouterProduit(HttpSession session,
	    ...
	    @GetMapping("/produitRuche")
		   public String produitRuche(Model model, HttpSession session) {
			   User loggedInUser = (User) session.getAttribute("loggedInUser");
			   if (loggedInUser != null) {
			   List<Produit> produits = produitService.getProduitsByIdUser(loggedInUser.getIdUser());
